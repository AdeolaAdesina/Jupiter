(function (d) {
    ...
    d.cColorSwitcher = function (b) {
        b = d.extend({ position: "left", switcherTitle: "No Colors", switcherColors: [], switcherTarget: "" }, b); var a = d('<div id="cColorSwitcher"></div>'), e = d("head"), f = d("body"); a.css({ transform: "translateY(-50%)" }).addClass("clearfix"); d('<div class="ccs--body"><h6>' + b.switcherTitle + "</h6><ul></ul></div>").appendTo(a).children("ul").append(function () {
            var a = "", c; for (c = 0; c < b.switcherColors.length; c++)"undefined" === typeof b.switcherColors[c].fgColor && (b.switcherColors[c].fgColor = "transparent"),
                a = a + '<li data-file-path="' + b.switcherColors[c].filepath + '"><span style="background-color: ' + b.switcherColors[c].bgColor + ';"></span><span style="transform: rotate(45deg); background-color: ' + b.switcherColors[c].fgColor + ';"></span></li>'; return a
        }).children("li").on("click", function () { var a = d(this); b.switcherTarget.length && b.switcherTarget.attr("href", a.data("file-path")); a.hasClass("active") || a.addClass("active").siblings().removeClass("active"); g.css("background-color", a.children("span").eq(0).css("background-color")); });
